...
export function PaymentSummary({ cart, subtotal, tax, discount, total, customer }: PaymentSummaryProps) {
  return (
    <div className="flex w-full md:w-[35%] lg:w-[400px] flex-col border-r border-slate-200 bg-slate-50 dark:bg-slate-900 dark:border-slate-800 h-full">
      {/* Header */}
      <div className="flex items-center justify-between border-b border-slate-200 bg-white p-6 shadow-sm dark:bg-slate-950 dark:border-slate-800">
        <div>
          <h2 className="text-xl font-bold text-slate-800 dark:text-white">Current Bill</h2>
      ...
      <div className="flex-1 overflow-y-auto p-4 scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-slate-700">
        <div className="space-y-3">
          {cart.map((item, index) => (
            <div 
              key={`${item.product_id}-${index}`} 
              className="flex items-center justify-between rounded-xl bg-white p-4 shadow-sm ring-1 ring-slate-100 dark:bg-slate-950 dark:ring-slate-800"
            >
              <div className="flex items-center gap-4">
                <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-slate-100 text-slate-500 font-bold dark:bg-slate-800 dark:text-slate-400">
                  {item.quantity}x
                </div>
                <div>
                  <div className="font-semibold text-slate-700 dark:text-slate-200 text-lg line-clamp-1">
                    {item.product?.name || 'Unknown Item'}
                  </div>
      ...
      {/* Totals Footer */}
      <div className="border-t border-slate-200 bg-white p-6 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] dark:bg-slate-950 dark:border-slate-800 z-10">
